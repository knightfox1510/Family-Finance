...
// Fixed OCR: proper error handling, progress states, size guard, retry UX
...
export function AddGroupExpense({
  groupId, groupName, currency, members, userId, ghostToken, fmt, onClose, onAdded,
}: Props) {
  ...
  const equalShare   = useMemo(
    () => includedList.length === 0 || totalAmount === 0 ? 0 : totalAmount / includedList.length,
  ...
  const itemizedTotal = useMemo(
    () => items.reduce((s, item) => s + (parseFloat(item.totalAmount) || 0), 0),
    [items]
  );
  ...
  const handleReceiptUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Reset state
    setIsScanning(true);
    setScanProgress('Uploading receipt…');
    setScanError(null);
    setError(null);

    // Client-side size guard (5 MB)
    if (file.size > 5 * 1024 * 1024) {
      setScanError('Image is too large (max 5 MB). Please use a lower resolution photo.');
      setIsScanning(false);
    ...
    try {
      ...
      if (uploadErr) {
        if (
          ...
          setScanError(
            'Receipt storage is not configured yet. ' +
            'Please create a public "receipts" bucket in Supabase Storage, ' +
            'or enter amounts manually.'
          );
        } else {
          setScanError(`Upload failed: ${uploadErr.message}`);
        }
        return;
      }
      ...
      if (!res.ok) {
        // Translate server hints into user-friendly messages
        const hint = data?.hint ?? '';
        let userMessage = data?.error ?? 'Could not read the receipt.';

        if (hint === 'bucket_not_public') {
          userMessage =
            'The receipt uploaded but scanning failed because the storage bucket is not public. ' +
            'Check Supabase Storage settings. You can enter amounts manually.';
        } else if (hint === 'gemini_error') {
          userMessage = 'AI scanning is temporarily unavailable. Please enter amounts manually.';
        } else if (hint === 'parse_error') {
          userMessage =
            'Could not read this receipt. Try a clearer, well-lit photo with ' +
            'the full receipt visible and no glare.';
        }
      ...
      // 4. Populate form from OCR result
      if (data.items && data.items.length > 0) {
        const newItems = data.items.map((item: { name: string; price: number }) => ({
          id: newItemId(),
          itemName: item.name,
          totalAmount: String(item.price),
          splitMode: 'equal' as const,
          memberIds: [], // user assigns in Step 2
          customAmounts: {},
        }));
        ...
        // Jump to Step 2 only when we got real line items
        // (single-item = just the total — stay on step 1 so user can confirm)
        if (data.items.length > 1) {
          setStep(2);
        }
      } else if (data.totalAmount > 0) {
        // Got a total but no line items — populate the amount only
        setAmount(String(data.totalAmount));
        setScanError(
          `Found total: ₹${data.totalAmount}. No line items detected — ` +
          'enter them manually or try a closer photo.'
        );
      } else {
        setScanError('No amounts found in the receipt. Try a clearer photo.');
      }

    } catch (err: any) {
      console.error('[OCR] Client error:', err);
      setScanError('Something went wrong. Please try again or enter amounts manually.');
    } finally {
      setIsScanning(false);
      setScanProgress('');
  ...
  const buildSplits = () => {
    ...
    // Itemized: flatten per-member across all items
    ...
    return Object.entries(splitMap).map(([uid, entries]) => ({
      userId:   uid,
  ...
  const inp: React.CSSProperties = {
    ...
    borderRadius: 12, color: C.textW, fontFamily: 'inherit', fontSize: 14,
    fontWeight: 500, padding: '11px 14px', outline: 'none',
    boxSizing: 'border-box', transition: 'border-color 0.15s',
  };
    ...
    <div style={{ display: 'flex', gap: 6, marginBottom: 20 }}>
      {[1, 2, 3].map((s) => (
        <div
          key={s}
          style={{
            height: 4, flex: s === step ? 2 : 1, borderRadius: 99,
    ...
    >
      <div
        style={{
          ...
          padding: '20px 24px 40px', maxWidth: 520, width: '100%',
      ...
      >
        ...
        {step === 1 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'center', gap: 4 }}>
                ...
                <input
                  ...
                  style={{
                    ...
                    color: C.textW, fontFamily: 'inherit', fontSize: 48, fontWeight: 900,
                    letterSpacing: '-0.04em', textAlign: 'center', maxWidth: 220, width: '100%',
                  }}
                />
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, marginTop: 16 }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    border: `1px solid ${isScanning ? C.border2 : C.accent}`,
                    background: isScanning ? C.surface2 : `${C.accent}15`,
                    color: isScanning ? C.text3 : C.accent,
                ...
                <div style={{ fontSize: 11, color: C.text3, textAlign: 'center', lineHeight: 1.5 }}>
                  Works best with flat, well-lit receipts · max 5 MB
                </div>
              </div>

              {/* Scan error — inside the amount card, below the button */}
              {scanError && (
                <div style={{
                  margin: '12px auto 0', maxWidth: 320, padding: '10px 14px',
                  borderRadius: 12, background: `${C.red}15`,
                  ...
                  <button
                    ...
                    style={{
                      ...
                      color: C.red, borderRadius: 99, padding: '4px 14px',
            ...
            {/* Submit error (separate from scan error) */}
            {error && (
              <div style={{
            ...
            {splitType === 'equal' && (
              <div>
                ...
                <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
                  {members.map((m, i) => (
                    <MemberAvatar
                      key={m.id} member={m} colorIndex={i}
                      selected={includedMembers.has(m.id)} size={48}
                      onToggle={() => setIncludedMembers((prev) => {
                ...
                {includedList.length > 0 && totalAmount > 0 && (
                  <div style={{
                    marginTop: 14, padding: '12px 16px', background: C.accentBg,
                    ...
                    <div style={{
                      ...
                      <input
                        ...
                        style={{
                          ...
                          fontWeight: 700, width: 80, textAlign: 'right',
                        }}
                      />
                    </div>
                  </div>
                ...
                <div style={{
                  ...
                  <span style={{
                    ...
                    {Math.abs(customTotal - totalAmount) <= 0.02
                      ? '✓ Matches'
                      : `${fmt(Math.abs(customTotal - totalAmount))} off`}
                  </span>
                </div>
              </div>
            )}
            ...
            {splitType === 'itemized' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <div style={{ fontSize: 12, color: C.text3 }}>
                  Add each item, then choose who shares it
                </div>
                    ...
                    <div key={item.id} style={{ background: C.surface2, borderRadius: 14, padding: '14px' }}>
                      ...
                      <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 10 }}>
                        <input
                          value={item.itemName}
                          onChange={(e) =>
                            setItems((prev) => prev.map((x, i) => i === idx ? { ...x, itemName: e.target.value } : x))
                          }
                        ...
                        <div style={{
                          ...
                          background: C.surface, borderRadius: 10, padding: '9px 12px', flexShrink: 0,
                        }}>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setItems((prev) => prev.map((x, i) => i === idx ? { ...x, totalAmount: e.target.value } : x))
                            }
                            placeholder="0"
                            style={{
                              background: 'transparent', border: 'none', outline: 'none',
                              color: C.textW, fontFamily: 'inherit', fontSize: 14,
                              fontWeight: 700, width: 70, textAlign: 'right',
                            }}
                          />
                        </div>
                        {items.length > 1 && (
                      ...
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
                        {members.map((m, mi) => (
                          <MemberChip
                            key={m.id} member={m} colorIndex={mi}
                        ...
                        <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
                          {(['equal', 'custom'] as const).map((mode) => (
                            <button
                              key={mode}
                              onClick={() =>
                                setItems((prev) => prev.map((x, i) => i === idx ? { ...x, splitMode: mode } : x))
                              }
                              style={{
                                padding: '5px 12px', borderRadius: 99,
                                border: `1px solid ${item.splitMode === mode ? C.accent : C.border2}`,
                                background: item.splitMode === mode ? C.accentBg : 'transparent',
                                color: item.splitMode === mode ? C.accent : C.text2,
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                          {item.memberIds.map((memberId) => {
                            ...
                            return (
                              <div key={memberId} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                <span style={{ fontSize: 12, color: C.text2, flex: 1 }}>
                                  {memberDisplayName(m)}
                                </span>
                                {item.splitMode === 'equal' ? (
                                  <span style={{ fontSize: 12, fontWeight: 700, color: C.textW }}>
                                    {fmt(perPerson)}
                                  </span>
                                ) : (
                                  <div style={{
                                    ...
                                    <input
                                      type="number"
                                      value={item.customAmounts[memberId] ?? ''}
                                      onChange={(e) =>
                                        setItems((prev) => prev.map((x, i) =>
                                          i === idx
                                            ? { ...x, customAmounts: { ...x.customAmounts, [memberId]: e.target.value } }
                                            : x
                                        ))
                                      }
                                      placeholder="0"
                                      style={{
                                        background: 'transparent', border: 'none', outline: 'none',
                                        color: C.textW, fontFamily: 'inherit', fontSize: 12,
                          ...
                          {item.splitMode === 'custom' && (
                            <div style={{ fontSize: 11, color: customOk ? C.green : C.red, marginTop: 4, fontWeight: 600 }}>
                              {customOk
                                ? `✓ ${fmt(itemTotal)} balanced`
                                : `${fmt(Math.abs(customSum - itemTotal))} ${customSum > itemTotal ? 'over' : 'remaining'}`}
                            </div>
                          )}
                        </div>
                      )}
                ...
                <button
                  ...
                  style={{
                    ...
                    color: C.text3, fontSize: 13, fontWeight: 600, cursor: 'pointer',
                ...
                <div style={{
                  ...
                  <span style={{ fontSize: 12, color: C.text2 }}>
                    Items total: {fmt(itemizedTotal)} of {fmt(totalAmount)}
                  </span>
                  <span style={{
                    fontSize: 12, fontWeight: 700,
