...
// Fixed: added ghostToken prop and forwards it as x-ghost-token header on fetch.
...
interface Props {
  ...
  members:     Profile[];
  userId:      string;
  ghostToken?: string;   // ← NEW: forwarded as x-ghost-token header
  fmt:         (n: number) => string;
  onClose:     () => void;
  onAdded:     () => void;
}
...
export function AddGroupExpense({
  ...
  const itemizedTotal = useMemo(() => itemizedEntries.reduce((s, e) => s + (parseFloat(e.amount) || 0), 0), [itemizedEntries]);
  ...
  const inp: React.CSSProperties = {
    ...
    fontWeight: 500, padding: '11px 14px', outline: 'none', boxSizing: 'border-box',
  ...
  const buildSplits = () => {
    ...
    return itemizedEntries
      ...
      .map((e) => ({ userId: e.userId, itemName: e.itemName.trim(), amount: parseFloat(e.amount) }));
  };
  ...
  const StepDots = () => (
    <div style={{ display: 'flex', gap: 6, marginBottom: 20 }}>
      {[1, 2, 3].map((s) => (
        <div key={s} style={{ height: 4, flex: s === step ? 2 : 1, borderRadius: 99, background: s <= step ? C.accent : C.border, transition: 'all 0.25s' }} />
      ))}
    </div>
  );
      ...
      <div style={{ background: C.surface, borderRadius: '24px 24px 0 0', padding: '20px 24px 40px', maxWidth: 520, width: '100%', maxHeight: '92vh', overflowY: 'auto', boxShadow: '0 -16px 60px rgba(0,0,0,0.6)' }} onClick={(e) => e.stopPropagation()}>
        ...
        {step === 2 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {([{ id: 'equal', label: '⚖️ Equal' }, { id: 'custom', label: '✏️ Custom' }, { id: 'itemized', label: '📋 Itemized' }] as const).map((opt) => (
            ...
            {splitType === 'itemized' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <div style={{ fontSize: 12, color: C.text3 }}>Assign specific items to specific people</div>
                {itemizedEntries.map((entry, idx) => (
                  <div key={idx} style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    <select value={entry.userId} onChange={(e) => setItemizedEntries((prev) => prev.map((x, i) => i === idx ? { ...x, userId: e.target.value } : x))}
                      ...
                      {members.map((m) => <option key={m.id} value={m.id}>{memberDisplayName(m).split(' ')[0]}</option>)}
                    </select>
                    <input value={entry.itemName} onChange={(e) => setItemizedEntries((prev) => prev.map((x, i) => i === idx ? { ...x, itemName: e.target.value } : x))} placeholder="Item name" style={{ ...inp, flex: 1, padding: '9px 12px' }} />
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: 2, background: C.surface2, borderRadius: 10, padding: '9px 10px' }}>
                      <span style={{ fontSize: 12, color: C.text3 }}>₹</span>
                      <input type="number" value={entry.amount} onChange={(e) => setItemizedEntries((prev) => prev.map((x, i) => i === idx ? { ...x, amount: e.target.value } : x))} placeholder="0"
                    ...
                    {itemizedEntries.length > 1 && (
                      <button onClick={() => setItemizedEntries((prev) => prev.filter((_, i) => i !== idx))} style={{ background: 'transparent', border: 'none', color: C.red, cursor: 'pointer', fontSize: 18, lineHeight: 1, padding: '4px', flexShrink: 0 }}>×</button>
                    )}
                  </div>
                ))}
                <button onClick={() => setItemizedEntries((prev) => [...prev, { userId, itemName: '', amount: '' }])} style={{ padding: '9px', borderRadius: 10, border: `1px dashed ${C.border2}`, background: 'transparent', color: C.text3, fontSize: 12, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                  + Add another item
                </button>
                <div style={{ padding: '10px 14px', borderRadius: 12, background: Math.abs(itemizedTotal - totalAmount) <= 0.02 ? C.greenBg : `${C.red}15`, border: `1px solid ${Math.abs(itemizedTotal - totalAmount) <= 0.02 ? C.green : C.red}33`, display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ fontSize: 12, color: C.text2 }}>Total: {fmt(itemizedTotal)} of {fmt(totalAmount)}</span>
                  <span style={{ fontSize: 12, fontWeight: 700, color: Math.abs(itemizedTotal - totalAmount) <= 0.02 ? C.green : C.red }}>{Math.abs(itemizedTotal - totalAmount) <= 0.02 ? '✓ Balanced' : `${fmt(Math.abs(itemizedTotal - totalAmount))} remaining`}</span>
                </div>
              </div>
            )}
        ...
        {step === 3 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              {splitType === 'itemized' && itemizedEntries.filter((e) => e.itemName && e.amount).map((e, i) => {
                ...
                return (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', fontSize: 13 }}>
                    <span style={{ color: C.text2 }}>{m ? memberDisplayName(m).split(' ')[0] : '?'} · {e.itemName}</span>
            ...
            {error && (
              <div style={{ padding: '10px 14px', borderRadius: 12, fontSize: 13, background: `${C.red}15`, border: `1px solid ${C.red}44`, color: C.red }}>
                {error}
              </div>
            )}
            <div style={{ display: 'flex', gap: 10 }}>
