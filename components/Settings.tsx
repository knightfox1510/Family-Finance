...
// App-wide configuration: household mode, partner names, categories, budgets,
// notifications, Telegram integration, data import/export, and household join.
...
export function Settings({ data, householdId, onSave, onExport, onImport, onJoinHousehold }: Props) {
  ...
  // FIX: joinId is now a proper top-level useState.
  // Previously it was declared inside an IIFE (immediately-invoked function expression)
  // inside JSX, which violates React's rules of hooks and causes TypeScript errors.
  const [joinId, setJoinId]       = useState('');

  const fileRef = useRef<HTMLInputElement>(null);

  // Re-sync local state whenever parent pushes new settings down
  useEffect(() => {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18, maxWidth: 720 }}>
      ...
      <Card>
        ...
        <p style={{ color: C.muted, fontSize: 13, margin: '0 0 14px' }}>
          Adjusts which features are available. You can switch at any time.
        </p>
          ...
          {modes.map((m) => {
            ...
            return (
              ...
              >
                ...
                <div style={{ color: C.text2, fontSize: 11, marginTop: 3, lineHeight: 1.4 }}>
                  {meta.description}
                </div>
              </button>
            );
      ...
      {/* Active Device Profile (two-partner modes only) */}
      {partnerB && (
        <Card style={{ border: `1px solid ${C.purple}44` }}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            {(['Partner A', 'Partner B'] as const).map((role) => (
              <Btn
                key={role}
                variant={currentCloudRole === role ? 'primary' : 'ghost'}
                style={{ flex: 1 }}
      ...
      <Card style={{ border: `1px solid ${C.teal}44` }}>
        ...
        <p style={{ color: C.text1, fontSize: 13, margin: '0 0 14px', lineHeight: 1.5 }}>
          {telegramLinked
            ? 'Your account is connected to the Telegram logging bot. Send it a message to log expenses on the go.'
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <input
            ...
            style={{
              background: telegramLinked ? `${C.bg}80` : C.bg,
          ...
          />
          {telegramLinked && (
            <Btn variant="danger" onClick={() => setS((x) => ({ ...x, telegramUsername: '' }))}>
              Unlink
            </Btn>
          )}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          {s.expenseCategories.map((c) => (
            <div key={c} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ color: C.text1, fontSize: 13, flex: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {c}
              </span>
              <Inp
                type="number"
      ...
      <Card>
        ...
        {/* joinId state is declared at the top of the component — no IIFE needed */}
        <div style={{ display: 'flex', gap: 8 }}>
          <Inp
            value={joinId}
